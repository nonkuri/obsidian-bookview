...
- **EPUB 対応（リフロー型・固定レイアウト型）**
  - 縦書きの日本語は縦書きのまま、ルビつきで表示します
  - 綴じ方向は EPUB 自身の `page-progression-direction` から確定的に決まるので、推測は不要です（右開きなら `←` が「次へ」）
  - 横書きの本は 2 段組みの見開き表示。縦書きは本文が全幅を右から左へ流れるため、それ自体が見開きに相当します
  - 目次パネルは現在位置の項目をハイライトします
  - 文字サイズの変更、ページ送り／連続スクロールの切り替え
  - 読んでいた位置は EPUB CFI として保存され、次に開いたときに正確に復元されます
- ネットワークアクセスなし。pdf.js 本体・Worker・CMap・EPUB レンダラはすべて `main.js` に同梱
  - EPUB はスクリプトと全ネットワーク通信を禁じる Content-Security-Policy のもとで描画するため、本の側からコードを実行したり外部へ通信したりはできません
...
- 新しく開く EPUB の既定値（ページ送り／連続スクロール、2 段組み、文字サイズ、1 行の長さ、行間、段の間隔）
